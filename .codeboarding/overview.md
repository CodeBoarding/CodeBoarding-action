...
graph LR
    ...
    Visual_Representation_Generator["Visual Representation Generator"]
    Structural_Change_Detector["Structural Change Detector"]
    Feedback_Telemetry_Handler["Feedback & Telemetry Handler"]
    Interaction_Orchestrator["Interaction Orchestrator"]
    Visual_Representation_Generator -- "Resolves file-to-component mapping" --> Structural_Change_Detector
    Visual_Representation_Generator -- "Requests interactive links" --> Interaction_Orchestrator
    Structural_Change_Detector -- "Queries analysis scope" --> Analysis_State_Manager
    Structural_Change_Detector -- "Triggers artifact rendering" --> Visual_Representation_Generator
    Feedback_Telemetry_Handler -- "Extracts execution metrics" --> Analysis_State_Manager
    Interaction_Orchestrator -- "Contextualizes UX links" --> Analysis_State_Manager
    Interaction_Orchestrator -- "Defines feedback payloads" --> Feedback_Telemetry_Handler
```
...
The CodeBoarding-action architecture is a CI/CD pipeline that transforms Git diffs and static analysis data into architectural visualizations, managing analysis state, generating visual reports, detecting structural changes, handling telemetry, and orchestrating user interactions.

### Analysis State Manager
Manages the lifecycle of architectural analysis data, including loading metadata, tracking quota usage, and determining whether to perform incremental or full analysis updates.
...
Transforms processed architectural data into visual formats, specifically handling the rendering of Mermaid.js diagrams and component-level documentation files.
...
- `scripts.build_component_files._block`:104-121
...
Analyzes the project structure and Git diffs to identify specific changes in methods, files, and component relationships that necessitate documentation updates.
...
- `scripts.diff_to_mermaid._diff_relations`:153-199
...
### Feedback & Telemetry Handler
Collects and transmits execution metadata and user feedback to external endpoints to monitor action performance and tool effectiveness.
...
Generates "Call to Action" (CTA) links and webview URLs that facilitate user interaction with the generated architectural reports within the GitHub environment.
